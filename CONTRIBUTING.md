...
- Keep product, safety, and interface decisions documented before building UI on top of them.
...
Every local action must be classified before it can run:

| Risk | Required review |
| --- | --- |
| read_only | User-requested action is enough. |
| low_write | Show plan and get approval. |
| high_write | Show exact diff or commands, create backup where possible, get approval. |
| danger | Require separate explicit approval and document rollback or restore path where possible. |
