...
Run your existing AI coding CLIs — and a Steam Deck support assistant — directly from Steam Deck Gaming Mode, through a [Decky Loader](https://decky.xyz) plugin.

Decky AI Assistant brings **Codex CLI**, **Claude Code**, `bash`, and your own custom commands into a controller-friendly terminal on the Deck, then layers a portable agent pack (skills, roles, and a local MCP tool server) on top so those CLIs can diagnose and fix Steam Deck problems without Decky taking over the shell.

It is **bring-your-own-account**: the plugin launches the CLIs you already log into. It never proxies model access, never reads your provider credentials, and adds no telemetry. The plugin may request Decky's root flag; command approval and sandboxing stay with the active CLI.

> **Status:** Terminal Mode is a working MVP you can use today. The agent layer ships risk metadata and a local MCP server with Deck diagnostics and fix-planning contracts. Live knowledge-pack search, `inspect_current_game`, and richer `propose_fix` wiring are in active development — see [Project status](#project-status).
...
- **Local MCP server (`deck-assistant`)** — a dependency-free JSON-RPC stdio server (`python -m deck_assistant_mcp serve`) exposing Deck diagnostics (`read_proton_logs`, `get_storage_report`, …), cited knowledge search, and concise fix plans. Requested writes stay in the active CLI terminal workflow.
- **Risk metadata** — local actions are risk-classified (`read_only` / `low_write` / `high_write` / `danger`) for display. Command approval belongs to Codex, Claude, or the custom CLI.
...
- The plugin may request Decky's root flag, but it does not run commands on its own initiative.
- Risk labels are informational. Writes stay in the active CLI workflow.
...
| Risk contracts: risk model, roles, tool policy | ✅ Complete & unit-tested |
...
| Assistant / Knowledge UI | 🗺️ Planned |
...
deck-assistant-core   risk classifier · diagnostics
...
The Deck runs everything: PTY sessions, local diagnostics, knowledge indexes, and MCP planning tools. Model access and command execution stay inside the user's official CLIs. MCP is the shared tool bridge the CLIs connect to.
...
Read [CONTRIBUTING.md](CONTRIBUTING.md) and the canonical [AGENTS.md](AGENTS.md) before starting. In short: keep each change to one coherent slice, keep the Decky frontend thin and risk/tool logic in the core/MCP packages, add tests around contracts and risk metadata, and update docs when contracts, commands, or risk behavior change. Releases follow the dev/stable policy in [RELEASING.md](RELEASING.md).
