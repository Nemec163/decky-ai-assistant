...
4. Prefer core/backend contracts before UI polish when safety, actions, sources, sessions, or CLI behavior are involved.
...
7. Route changes to risk ceilings, approval-token boundaries, staged-action semantics, or execution permissions through `deck-safety-reviewer` before finalizing.
8. Update tests and docs in the same slice when stable behavior changes.
9. Run the narrowest relevant checks from `docs/operations.md`.
10. End with a development handoff that includes `slice_goal`, `files_in_scope`, `files_changed`, `verification`, `risk_notes`, `commit_status`, `allowed_next_role`, and `blocked_condition`.
11. Create one meaningful commit when the slice is complete; if blocked before commit, leave `commit_status` set to `in_progress` or `ready_to_commit`.
...
- Keep runtime write behavior behind staged actions and Decky-side approval tokens.
