...
- The Steam Deck runs the Decky plugin, PTY sessions, local diagnostics, knowledge indexes, and command execution through the active CLI.
...
- Repo-local agent pack assets define portable skills, role labels, commands, adapter templates, and MCP config examples so development and runtime agents share one risk model.
...
| MCP server | Stable static tool contract catalog, risk metadata, and in-process dispatcher shell with optional injected knowledge index, read-only diagnostics handlers/readers, and fix-planning hooks, plus a dependency-free newline-delimited JSON-RPC 2.0 stdio transport (`python -m deck_assistant_mcp serve`) that wires real bounded diagnostics readers; full knowledge-index wiring is planned | [packages/mcp-server/src/deck_assistant_mcp/contracts.py](../packages/mcp-server/src/deck_assistant_mcp/contracts.py), [packages/mcp-server/src/deck_assistant_mcp/dispatcher.py](../packages/mcp-server/src/deck_assistant_mcp/dispatcher.py), [packages/mcp-server/src/deck_assistant_mcp/server.py](../packages/mcp-server/src/deck_assistant_mcp/server.py), [ROADMAP.md](../ROADMAP.md) |
...
8. The CLI calls `deck-assistant-mcp` for knowledge search, diagnostics, and fix planning.
9. Agent pack roles label whether the CLI is planning, diagnosing, or curating sources.
10. Read-only tools return structured results with citations.
11. Write actions are shown as exact commands/diffs when available.
12. When execution is requested, the active CLI and terminal path handle approval/sandbox behavior.
...
| Audit log | Sanitized diagnostics events | Planned |
...
- MCP exposes read, diagnostics, and fix-planning tools; requested writes stay with the active CLI and normal terminal tooling.
- CLI permission bypass is disabled by default, visible in Settings, classified as `danger`, and maps only to documented CLI-native no-approval modes. The Decky plugin metadata may request the `root` flag when needed.
...
- `AGENTS.md` and `agent-pack/tool-policy.json` define the shared risk model; target adapters must not weaken it.
...
- The MCP server may expose local tools; requested writes stay in the active CLI workflow.
...
- Native assistant pack installation may write user-local CLI extension files after a user request; it must not run provider CLIs or read auth stores.
