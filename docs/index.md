...
- The project will combine Decky UI, PTY terminal sessions, local MCP tools, Agent Skills, knowledge packs, and explicit action approvals.
...
- Repo-local agent workflows live in [agent-pack/manifest.json](../agent-pack/manifest.json) with portable skills for game, storage, Flatpak, knowledge, and safety workflows, custom agent roles, commands, MCP templates, and adapter manifests.
- Initial shared core contracts exist for PTY sessions including Claude managed-workspace cwd selection, managed CLI setup plans with user-local Node/npm bootstrap, user-local native assistant pack installs including Claude direct user-level skills/agents, per-profile permission-bypass planning, staged actions, risk classification, CLI profile detection, knowledge packs/search/local-folder manifest builds, source registry state, persisted SQLite FTS5/BM25 knowledge indexes, read-only diagnostics reports, MCP knowledge dispatch backed by an injected knowledge index, and MCP pending-action staging backed by an injected in-memory store.
...
| Contribute or review changes | [../CONTRIBUTING.md](../CONTRIBUTING.md) | Workflow, safety review, PR checklist |
...
| Test core contracts | `PYTHONPATH=packages/core/src python3 -m unittest discover -s packages/core/tests` | Runs stdlib unit tests for PTY session, risk, staged-action, CLI profile, knowledge/source registry, and diagnostics contracts. |
...
| [CONTRIBUTING.md](../CONTRIBUTING.md) | Contributor workflow and review gates. |
...
- The project will eventually execute local commands on a Steam Deck; write actions must require explicit approval.
